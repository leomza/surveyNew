  ...
  deleteSurveyForUser(surveyId) {
    try {
      this.users.forEach((user) => {
        user.createdSurveys = user.createdSurveys.filter(
          (createdSurvey) => createdSurvey !== surveyId
        );
      });

      this.users.forEach((user) => {
        user.answeredSurveys = user.answeredSurveys.filter(
          (answeredSurveys) => answeredSurveys !== surveyId
        );
      });
